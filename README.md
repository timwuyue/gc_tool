...
**在 ComfyUI 页面右上角添加一个浮动按钮（"一掌劈开天"）**，点开即一个轻量聊天窗口，直连本机运行的 DSH（DeepSeek Harness）本体（`http://127.0.0.1:3080`）——即**完全体的 AI agent**（同模型 + 全套工具：读文件、跑命令、调 MCP、控制远程 GPU 等），全程无需任何 API key。
